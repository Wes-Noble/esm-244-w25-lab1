...
What is site order?
Correlation between legal status and species type?
Many many species?
Which trees require the largest plot size?

How many counts of each observation by legal_status? Focus on the ghiest trees?
```{r}
top_5 = sfo_trees_df |>
...
remove columns that we dont want (select)
filter out all trees that are not pine and oak (filter)
summarize the counts of the oaks and pine
...
```{r}
ggplot()+
  geom_point(data = oak_pine_df, aes(x=latitude, y=longitude, color = type)) +
  theme_bw() +
...
ggplot()+
  ...
  geom_sf(data=oak_pine_sf, aes(color=spp_sci),
          size=0.5)+
  theme_bw()+
